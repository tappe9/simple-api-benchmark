...
def git(root: Path, *args: str, data: bytes | None = None, environment=None) -> str:
    try:
        result = subprocess.run(["git", *args], cwd=root, input=data, stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE, env=environment, timeout=90, check=False)
...
def publish(report: dict, root: Path, *, expected_context: dict, environment=None) -> str:
    ...
    require(git(root, "rev-parse", "HEAD") == source, "publication checkout must match measured source")
    require(git(root, "rev-parse", "HEAD^{tree}") == report["metadata"]["source_tree"], "source tree mismatch")
    # No worktree changes or user-staged files may enter the transaction.
    require(not git(root, "status", "--porcelain", "--untracked-files=normal"), "publication needs clean source")
    remote = git(root, "ls-remote", "--exit-code", "origin", "refs/heads/main", environment=environment)
    ...
    with tempfile.TemporaryDirectory(prefix="sab-publish-") as directory:
        ...
        env.update(GIT_INDEX_FILE=str(Path(directory) / "index"),
        ...
        for filename, content in updates.items():
            ...
            git(root, "update-index", "--add", "--cacheinfo", "100644", blob, filename, environment=env)
        tree = git(root, "write-tree", environment=env)
        commit = git(root, "commit-tree", tree, "-p", source,
...
def main() -> int:
    try:
        ...
        require(git(ROOT, "remote", "get-url", "origin") in (f"https://github.com/{REPOSITORY}", f"https://github.com/{REPOSITORY}.git"),
