...
def validate_registry(data: dict) -> dict:
    object_fields(data, ("schema_version", "active_cohort", "implementations", "cohorts"), "registry")
    ...
    for spec in specs:
        object_fields(
            spec,
            ("id", "language", "framework", "display_name", "source_path", "version_fields",
             "acceptance_test", "failure_test"),
            "implementation registration",
        )
        identifier = spec["id"]
        require(type(identifier) is str and re.fullmatch(r"[a-z][a-z0-9]*(?:-[a-z0-9]+)+", identifier), "invalid implementation ID")
        identifiers.append(identifier)
        for key in ("language", "framework", "display_name"):
            value = spec[key]
            require(type(value) is str and 0 < len(value.strip()) <= 128 and all(ord(c) >= 32 for c in value), "invalid " + key)
        path = spec["source_path"]
        require(type(path) is str and re.fullmatch(r"apps/[a-z][a-z0-9]*(?:-[a-z0-9]+)+", path), "invalid source path")
        sources.append(path)
        for key in ("acceptance_test", "failure_test"):
            value = spec[key]
            if key == "failure_test" and value is None:
                continue
            require(type(value) is str and re.fullmatch(r"tests/test_[a-z0-9_]+\.py", value), "invalid test path")
        fields = spec["version_fields"]
        require(type(fields) is list and bool(fields), "required version fields missing")
        require(all(type(key) is str and re.fullmatch(r"[a-z][a-z0-9_-]*", key) for key in fields), "invalid version field")
    ...
    require(type(cohorts) is dict and LEGACY_COHORT in cohorts, "legacy cohort must remain registered")
    for identifier, cohort in cohorts.items():
        require(type(identifier) is str and re.fullmatch(r"[a-z][a-z0-9-]*-v[1-9][0-9]*", identifier), "cohort ID must be versioned")
        ...
        require(all(type(member) is str and member in identifiers for member in members), "unknown cohort member")
        require(len(members) == len(set(members)), "duplicate cohort member")
    require(type(data["active_cohort"]) is str and data["active_cohort"] in cohorts, "unknown active cohort")
    return data
...
def report_members(report: dict) -> tuple[str, ...]:
    ...
    if version == 1:
        ...
        identity = object_fields(report.get("benchmark"), ("definition", "cohort"), "benchmark identity")
        cohort_id = identity["cohort"]
        require(type(cohort_id) is str and cohort_id in REGISTRY["cohorts"], "unknown benchmark cohort")
        require(identity["definition"] == REGISTRY["cohorts"][cohort_id]["definition"] == DEFINITION_ID, "unknown benchmark definition/cohort pairing")
...
def generated_files() -> dict[str, str]:
    ...
    projection = {
        ...
        "implementations": [
            {key: value for key, value in spec.items() if key not in ("acceptance_test", "failure_test")}
            for spec in REGISTRY["implementations"]
        ],
    }
    javascript = "// Generated by python -m benchmark.registry --write; do not edit.\n"
    javascript += "export const REGISTRY = " + json.dumps(projection, ensure_ascii=True, allow_nan=False, sort_keys=True, indent=2) + ";\n"
    ...
    for spec, target in zip(REGISTRY["implementations"], targets):
        ...
        if spec["failure_test"] is not None:
            make.append("\t@$(PYTHON) -m unittest discover -s tests -p " + Path(spec["failure_test"]).name)
...
def check_generated(root: Path = ROOT) -> None:
    for name, expected in generated_files().items():
        ...
        require(path.read_bytes() == expected.encode("utf-8"), "stale generated projection: " + name)
...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument("--check", action="store_true", help="verify sources and deterministic projections (default)")
    mode.add_argument("--write", action="store_true", help="regenerate only the Make and JavaScript projections")
