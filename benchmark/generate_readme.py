...
def render(report: dict | None, locale: str) -> str:
    ...
    lines += [
        ...
        (
            ...
            if locale == "en"
            else "↑ 多いほど高速、↓ 少ないほど良好です。メモリはAPIコンテナだけの観測値で、"
...
def main() -> int:
    ...
    parser.add_argument(
        "--check", action="store_true", help="check generated sections without writing"
    )
    ...
    try:
        ...
        for path, value in updates.items():
            if args.check:
                require(
                    path.read_text(encoding="utf-8") == value,
                    f"stale generated section: {path.name}",
                )
            else:
