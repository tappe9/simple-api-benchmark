"""Regression tests for Axum JSON and container acceptance assertions."""
...
import test_rust_axum_service as service
...
class ContainerProcessTests(unittest.TestCase):
    def run_commands(self, process_rows):
        state = {
            ...
            "Path": "/usr/local/bin/rust-axum", "Args": [],
        ...
        def execute(command, **kwargs):
            if command == ["docker", "compose", "ps", "--quiet", "rust-axum"]:
                output = "container-id\n"
    ...
    def test_one_server_with_healthcheck_is_accepted(self):
        processes = "8824 /usr/local/bin/rust-axum\n9000 /usr/local/bin/rust-axum healthcheck\n"
