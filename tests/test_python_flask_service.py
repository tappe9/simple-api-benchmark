...
def run(command: list[str], *, cwd: Path = ROOT, timeout: int = 600, check: bool = True):
...
def focused_tests() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="python-flask-tests-") as directory:
        ...
        run([python, "-m", "pip", "install", "--require-hashes", "--only-binary=:all:", "-r", "requirements-dev.lock"], cwd=APP)
...
def running_service() -> str:
    ...
    require(state["State"]["Health"]["Status"] == "healthy", "Flask container unhealthy")
    ...
    require(state["Path"] == "python" and state["Args"] == ["-m", "benchmark_api.server"], "unexpected server wrapper")
...
def endpoints() -> None:
    ...
    for value in ("invalid", "42junk", "1.0", "1e2", "0x2a", "1_000", "9223372036854775808", "-9223372036854775809"):
...
def startup_failure() -> None:
    result = run([
        "docker", "compose", "run", "--rm", "--no-deps", "-e", "DATABASE_HOST=missing.invalid",
        SERVICE,
    ], timeout=45, check=False)
    require(result.returncode != 0, "service started without a reachable database")
    require("password" not in (result.stdout + result.stderr).lower(), "startup failure leaked credentials")


def graceful_shutdown(container: str) -> None:
    run(["docker", "kill", "--signal=TERM", container])
    result = run(["docker", "wait", container], timeout=30)
    require(result.stdout.strip() == "0", f"SIGTERM exit code was {result.stdout.strip()!r}")
...
def main() -> int:
    try:
        ...
        startup_failure()
        run(["docker", "compose", "up", "--detach", "--wait", "--wait-timeout", "60", SERVICE], timeout=180)
        ...
        run(["docker", "compose", "down", "--remove-orphans", "--volumes"], timeout=120, check=False)
