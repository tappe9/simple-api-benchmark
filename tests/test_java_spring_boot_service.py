...
def count_value(value):
    require(type(value) is str and re.fullmatch(r"[0-9]+\s*", value) is not None, "invalid database session count")
    return int(value)
...
def stopped_state(container):
    try:
        ...
        require(all(state[key] is False for key in ("Running", "Restarting", "Dead", "OOMKilled")), "container is not safely stopped")
        require(state["Status"] == "exited", "container did not exit")
        require(type(state["ExitCode"]) is int, "missing integer exit status")
        require(type(container["RestartCount"]) is int and container["RestartCount"] == 0, "container restarted")
        return state
...
def validate_failed_startup(container, logs):
    require(stopped_state(container)["ExitCode"] == 1, "expected an actual application startup failure")
    require(type(logs) is str and "application startup failed" in logs, "missing application failure evidence")
    lowered = logs.lower()
    require(not any(value in lowered for value in ("password", "secret-67", "jdbc:", "select ", "at org.")), "startup log disclosed internal details")


def dependency_hashes():
    return {name: hashlib.sha256((APP / name).read_bytes()).hexdigest() for name in DEPENDENCY_FILES}
...
def static_checks():
    ...
    require(bool(images) and all(re.search(r"@sha256:[0-9a-f]{64}$", image) for image in images), "Docker bases must be digest pinned")
    ...
    require("calculate(n - 1) + calculate(n - 2)" in fibonacci, "CPU workload must remain direct recursion")
    require("Fibonacci.calculate(30)" in (source / "ApiController.java").read_text(), "CPU calculation must occur per request")
...
def host_checks():
    ...
    versions = pinned_versions(APP)
    for name, version in (("spring-boot", versions["spring-boot"]), ("tomcat-embed-core", versions["tomcat"]), ("postgresql", versions["postgresql"]), ("HikariCP", versions["hikaricp"]), ("jackson-databind", versions["jackson"])):
        require(f"{name}-{version}.jar" in libraries, "runtime distribution does not match locked metadata")
    require(not any(name.startswith(("junit-", "mockito-", "spring-test-")) for name in libraries), "test dependencies entered the production distribution")
...
def container_checks():
    ...
    def sql(statement):
        return dc("exec", "-T", "postgres", "psql", "--username", "benchmark", "--dbname", "benchmark", "--no-psqlrc", "--tuples-only", "--no-align", "--set", "ON_ERROR_STOP=1", "--command", statement).strip()
    ...
    try:
        ...
        require(re.fullmatch(r"[0-9a-f]{64}", identifier) is not None, "missing full Java container identity")
        state = inspect(identifier)
        ...
        require(any(value.startswith("no-new-privileges") for value in state["HostConfig"]["SecurityOpt"]), "no-new-privileges is missing")
        ports = state["HostConfig"]["PortBindings"]
        require(ports == {"8080/tcp": [{"HostIp": "127.0.0.1", "HostPort": "8080"}]}, "unexpected Java host ports")
        require(set(state["NetworkSettings"]["Networks"]) == {project + "_benchmark"}, "Java escaped its owned benchmark network")
        database = inspect(dc("ps", "-q", "postgres").strip())
        require(not database["HostConfig"]["PortBindings"], "PostgreSQL must not publish a host port")
        validate_processes(state, run(["docker", "top", identifier, "-eo", "pid,args"]))
        runtime = run(["docker", "exec", identifier, "java", "-XshowSettings:properties", "-version"])
        ...
        validate_processes(inspect(identifier), run(["docker", "top", identifier, "-eo", "pid,args"]))
        ...
        for index, override in enumerate(("DATABASE_PASSWORD=secret-67", "DATABASE_HOST=127.0.0.1", "DATABASE_PORT=65536")):
            name = f"{project}-failure-{index}"
            dc("run", "--detach", "--no-deps", "--name", name, "--env", override, SERVICE, timeout=30)
            ...
            result = subprocess.run(["docker", "logs", name], capture_output=True, text=True, timeout=10, check=True)
            validate_failed_startup(failed, logs + result.stderr)
            require(sql(SESSION_COUNT) == "0", "failed startup left database sessions open")
        print("Java production acceptance: live SQL, BIGINT, pool, limits, process isolation, failures and shutdown passed")
        ...
        try:
            ...
            require(not run(["docker", "ps", "-aq", "--filter", label]).strip(), "owned acceptance containers remain")
            require(not run(["docker", "network", "ls", "-q", "--filter", label]).strip(), "owned acceptance networks remain")
        except Exception as cleanup:
            if failure is not None:
                raise RuntimeError(f"Java acceptance and owned cleanup both failed: {failure}; {cleanup}") from failure
            raise
