...
class JavaVersionTests(unittest.TestCase):
    ...
    def test_valid_committed_metadata_requires_no_native_toolchain(self):
        ...
        self.assertEqual(
            set(versions),
            {"java", "spring-boot", "gradle", "tomcat", "postgresql", "hikaricp", "jackson"},
        )
    ...
    def test_changed_boot_bom_cannot_silently_disagree_with_lock(self):
        self.reject_change(
            "build.gradle", "spring-boot-dependencies:4.1.1", "spring-boot-dependencies:4.1.2"
        )
    ...
    def test_snapshot_dependency_is_rejected(self):
        self.reject_change(
            ...
            "spring-boot-dependencies:4.1.1-SNAPSHOT=",
        )
    ...
    def test_duplicate_locked_dependency_is_rejected(self):
        ...
        line = next(
            ...
            if line.startswith("org.postgresql:postgresql:")
        )
    ...
    def test_distribution_checksum_cannot_be_removed(self):
        self.reject_change(
            "gradle/wrapper/gradle-wrapper.properties", "distributionSha256Sum=", "removedChecksum="
        )
    ...
    def test_wildcard_artifact_trust_is_rejected(self):
        self.reject_change(
            "gradle/verification-metadata.xml",
            "</configuration>",
            '<trusted-artifacts><trust group=".*" regex="true"/></trusted-artifacts></configuration>',
        )
