#!/usr/bin/env python3
"""Acceptance checks for the Rust / Axum API service."""
...
class CheckFailure(RuntimeError):
    """Raised when the Rust / Axum service violates its contract."""
...
def check_static_contract() -> None:
    ...
    service = compose_service_block(compose, "rust-axum")
    require("<<: *api-defaults" in service, "rust-axum does not inherit shared API defaults")
    require("context: ./apps/rust-axum" in service, "rust-axum build context is incorrect")
    require("<<: *database-environment" in service, "rust-axum database environment is missing")
    require("/usr/local/bin/rust-axum" in service, "rust-axum health check is missing")
    ...
    require(
        'ENTRYPOINT ["/usr/local/bin/rust-axum"]' in dockerfile,
    ...
    require('#[tokio::main(flavor = "current_thread")]' in main_source,
            "Tokio executor must be explicitly single-threaded")
    require(".with_graceful_shutdown(shutdown)" in main_source,
            "in-flight requests must drain before closing the pool")
    production = main_source.split("#[cfg(test)]", 1)[0] + production_api
    for forbidden in ("spawn_blocking", "new_multi_thread", 'flavor = "multi_thread"'):
        require(forbidden not in production, "CPU work must not be offloaded or parallelized")
    ...
    require(
        re.search(r"(?m)^test-rust-axum\s*:", generated) is not None,
        "generated Makefile target is missing: test-rust-axum",
    )
...
def check_container_contract() -> str:
    container_id = run(["docker", "compose", "ps", "--quiet", "rust-axum"]).stdout.strip()
    require(bool(container_id), "rust-axum container is not running")
    ...
    require(state["Path"] == "/usr/local/bin/rust-axum" and state["Args"] == [],
    ...
    require(sum(len(fields) == 2 and fields[0].isdigit()
                and fields[1] == "/usr/local/bin/rust-axum" for fields in commands) == 1,
            "expected one server process")
    run(["docker", "compose", "exec", "-T", "rust-axum", "sh", "-c",
...
def check_startup_failure() -> None:
    failed = run(["docker", "compose", "run", "--rm", "--no-deps", "-e", "DATABASE_PORT=1",
                  "rust-axum"], timeout=20, check=False)
...
def check_shutdown(container_id: str) -> None:
    ...
    run(["docker", "compose", "stop", "--timeout", "15", "rust-axum"])
...
def check_dynamic_contract() -> None:
    ...
    project = config["name"]
    service = config["services"]["rust-axum"]
...
def main() -> int:
    ...
    try:
        ...
        print(f"Rust / Axum service check failed: {exc}", file=sys.stderr)
        return 1

    mode = "static contract" if args.static else "Rust / Axum service"
