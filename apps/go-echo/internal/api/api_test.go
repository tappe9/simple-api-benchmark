...
func TestDatabaseEndpointReturnsTheQueriedItem(t *testing.T) {
	querier := &stubQuerier{row: stubRow{item: itemResponse{ID: 42, Name: "Item 42", Price: 4200}}}
...
func TestDatabaseEndpointRejectsInvalidIDsWithoutQuerying(t *testing.T) {
	for _, path := range []string{"/db/not-an-integer", "/db/1.5"} {
		querier := &stubQuerier{}
		response := performRequest(t, NewRouter(querier), path)
		...
		if got, want := decodeJSON[errorResponse](t, response), (errorResponse{Error: "invalid id"}); got != want {
			t.Fatalf("%s response = %#v, want %#v", path, got, want)
		}
		if querier.called {
			t.Fatalf("database query executed for invalid path %s", path)
		}
	}
}
