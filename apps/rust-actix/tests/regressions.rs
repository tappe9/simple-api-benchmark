use rust_actix::{database::{DatabaseConfig, DatabaseConfigError}, healthcheck::check_health};
use std::{io::{BufRead, BufReader, Write}, net::TcpListener, thread, time::Duration};

#[test]
fn database_debug_does_not_expose_password() {
    let config = DatabaseConfig {
        host: "postgres".into(), port: 5432, database: "benchmark".into(),
        user: "benchmark".into(), password: "sensitive-test-password".into(),
    };
...
fn healthcheck_rejects_additional_response_fields() {
    ...
    server.join().unwrap();
    assert!(result.is_err(), "healthcheck accepted fields outside the readiness contract");
}
